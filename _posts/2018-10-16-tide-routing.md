...
This tiny API is meant as a stand-in for more complex back ends. The main point of interest is that `Database` a *handle* to a database, meaning that it is `Clone` (and uses an `Arc` under the hood). We'll see why that's important later on.
